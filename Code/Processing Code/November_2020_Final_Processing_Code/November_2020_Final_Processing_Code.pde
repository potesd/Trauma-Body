...
import processing.sound.*;
   ...
   boolean approachplayed = false; // sets boolean for touch having been triggered last
...
void setup()
{
  ...
  approach = new SoundFile(this, "approach.aiff");
  level1 = new SoundFile(this, "level1.aiff");
...
void draw()
{
      ...
      int volumesize = int(map(volume, 0, 0.5, 1, 100));
      println(volumesize);
  ...
  for(Hand hand: leap.getHands()){
    ...
    strokeWeight(1);
        ...
        float value = round(handvalue);
  
        println("Value" + value);
        float valuereal = map(value,0,90,90,0);
         if (handvalue >= 0 && handvalue <= 90){
        myPort.write((byte)(valuereal));
        }
        
           
         
        if (handvalue <= 40){
          if(approach.isPlaying() == false ){
            approach.play();
          } else if (handvalue > 40 ) {
            if(approach.isPlaying() == true){
            approach.stop();
            level1.stop();
            level2.stop();
            level3.stop();
        ...
        //       if (volume <= 10){
        //  if(approach.isPlaying() == false || level1.isPlaying() == false || level2.isPlaying() == false || level2.isPlaying() == false){
        //    level1.play();
        //  } else if (volume > 10 ) {
        //    if(level1.isPlaying() == true){
        //    level1.stop();
        //    level2.stop();
        //    level3.stop();
        //    approach.stop();
